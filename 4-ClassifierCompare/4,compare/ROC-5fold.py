...
plt.plot(P, R, color='black',
         label=r'RandomForest (AUC = %0.4f)' % (roc_auc),linestyle='-',
         lw=2, alpha=1)
...
# 2
PAndRAttribute = []
ReadMyCsv(PAndRAttribute, 'FprAndTprDecisionTree.csv')
P = []
R = []
counter = 0
while counter < len(PAndRAttribute):
...
plt.plot(P, R, color='orange',
         label=r'DecisionTree (AUC = %0.4f)' % (roc_auc),linestyle='--',
         lw=2, alpha=1)
...
# 4
PAndRAttribute = []
ReadMyCsv(PAndRAttribute, 'FprAndTprLogistic.csv')
P = []
R = []
counter = 0
while counter < len(PAndRAttribute):
...
plt.plot(P, R, color='deepskyblue',
         label=r'Logistic (AUC = %0.4f)' % (roc_auc),linestyle=':',
         lw=2, alpha=1)

# 5
PAndRAttribute = []
ReadMyCsv(PAndRAttribute, 'FprAndTprNaiveBayes.csv')
P = []
R = []
counter = 0
while counter < len(PAndRAttribute):
...
plt.plot(P, R, color='green',
         label=r'NaiveBayes (AUC = %0.4f)' % (roc_auc),linestyle=':',
         lw=2, alpha=1)
...
#plt.legend(bbox_to_anchor=(0.385, 0.36), fontsize='large')
