...
while counter < 5:
    ...
    # TrainPositive,TestPositive由csv读取而来
    TrainPositiveFeature = []
    TrainList = []
    TrainName = 'TrainName' + str(counter) + '.csv'
    ReadMyCsv(TrainList, TrainName)
    counterT = 0
    while counterT < len(TrainList):
        pair = []
        pair.extend(AllNodeFeatureNum[int(TrainList[counterT][0])])
        pair.extend(AllNodeFeatureNum[int(TrainList[counterT][1])])
        TrainPositiveFeature.append(pair)
        counterT = counterT + 1

    TestPositiveFeature = []
    TestList = []
    TestName = 'TestName' + str(counter) + '.csv'
    ReadMyCsv(TestList, TestName)
    counterT = 0
    while counterT < len(TestList):
        pair = []
        pair.extend(AllNodeFeatureNum[int(TestList[counterT][0])])
        pair.extend(AllNodeFeatureNum[int(TestList[counterT][1])])
        TestPositiveFeature.append(pair)
        counterT = counterT + 1
