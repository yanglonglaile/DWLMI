...
FinalDiseaseFeature = []
ReadMyCsv(FinalDiseaseFeature, 'FinalDiseaseFeature.csv')         # 部分由Gaussian部分生成
...
StorFile(FinalDiseaseFeatureNum, 'FinalDiseaseFeatureNum.csv')


FinalLncKmerNum = []
counter = 0
while counter < len(FinalLncKmer):
    counter1 = 0
    while counter1 < len(AllNode):
        if FinalLncKmer[counter][0] == AllNode[counter1][0]:
            pair = []
            pair.append(str(counter1))
            pair.extend(FinalLncKmer[counter][1:])
            FinalLncKmerNum.append(pair)
            break
        counter1 = counter1 + 1
    print(counter)
    counter = counter + 1

StorFile(FinalLncKmerNum, 'FinalLncKmerNum.csv')
